...
You spot a bug or want a feature. You hit ✦ in your toolbar, talk through what you want while showing it on screen, and stop. Claude Brief saves a zip locally in `~/Downloads/brief/` and adds it to your inbox.

Capture as many as you like through the day — they queue up. Then copy the inbox prompt and paste it into Claude Code. Claude reads each brief, files a real ticket in your tracker, groups duplicates, and deletes the source brief once it's filed.
...
Click the ✦ icon, hit **Export** once you've recorded at least one brief, and paste the prompt into Claude Code Desktop whenever you like. Claude reads every exported brief, files the tickets, groups duplicates, and deletes each brief once its ticket is filed. Briefs you haven't recorded yet stay in your list for later.
...
No. A brief is a self-contained folder with the filing skill inside, so any local agent that can read files and reach your tracker works — Claude Code Desktop, Cursor, Cline, and so on. Without some local agent to read them, the briefs just sit there unprocessed.
...
The zip is self-contained — it includes the skill. Send it to them; they unzip it and paste `Read brief-<id>/skill/SKILL.md and follow it to file this as a ticket.` into any local agent (Claude Code Desktop, Cursor, …) and it'll process it on their machine.
...
The skill ships inside each brief, so there's nothing in `~/.claude/skills/` to update. To get skill or extension improvements, pull the latest repo and reload the unpacked extension at `chrome://extensions` — new briefs you record will bundle the newest skill automatically.
